...
class RM_Field_Factory_Addon {
    ...
    public function __construct($db_field, $opts, $prevent_value_update = false,$widget=false) {
    ...
    public function create_select_field() {
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page) {
            ...
            $this->opts['value'] = empty($select_field) ? $this->opts['value'] : $select_field;
        }
    ...
    public function create_MultiDropdown_field() {
        ...
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page) {
            $current_user = wp_get_current_user();
            $multi_dd = get_user_meta($current_user->ID, $this->get_user_meta_key(), true);
            $this->opts['value'] = empty($multi_dd) ? $this->opts['value'] : $multi_dd;
        }
    ...
    public function create_base_field() {
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev'])) {
            ...
            $this->opts['value'] = empty($user_base_info) ? $this->opts['value'] : $user_base_info;
        }
    ...
    public function create_mobile_field() {
        ...
        if($this->field_options->format_type=='international'){
            ...
            if($ca_state_type=='america'){
                ...
                if(!empty($this->field_options->country_match)){
                    if($embed){
                         echo "<script>if (typeof telDuplicate_" . $this->opts['id'] . " === 'undefined') { telDuplicate_" . $this->opts['id'] . " = true; const el = document.querySelector('#" . $this->opts['id'] . "'); var iti_" . $this->opts['id'] . " = window.intlTelInput(el, {initialCountry:'us',preferredCountries:".$preferred_countries.",".$placeholder_sett."}); jQuery(el).on('keyup', function(e) { const check = iti_" . $this->opts['id'] . ".isValidNumber() ? 1 : 0; el.dataset.validnumber = check; el.dataset.fullnumber = iti_" . $this->opts['id'] . ".getNumber(intlTelInputUtils.numberFormat.E164); jQuery(el).closest('.rminput').find('.selected-flag').addClass('disable-flag'); } }); }</script>";
                    }
                    else
                    {
                ...
                {   
                     if($embed){
                        echo "<script>if (typeof telDuplicate_" . $this->opts['id'] . " === 'undefined') { telDuplicate_" . $this->opts['id'] . " = true; const el = document.querySelector('#" . $this->opts['id'] . "'); var iti_" . $this->opts['id'] . " = window.intlTelInput(el, {initialCountry:'us',preferredCountries:".$preferred_countries.",".$placeholder_sett."}); jQuery(el).on('keyup', function(e) { const check = iti_" . $this->opts['id'] . ".isValidNumber() ? 1 : 0; el.dataset.validnumber = check; el.dataset.fullnumber = iti_" . $this->opts['id'] . ".getNumber(intlTelInputUtils.numberFormat.E164); }); }</script>"; 
                     }
                     else
                     {
                ...
                if($embed){
                    echo "<script>if (typeof telDuplicate_" . $this->opts['id'] . " === 'undefined') { var telDuplicate_" . $this->opts['id'] . " = true; const el = document.querySelector('#" . $this->opts['id'] . "'); var iti_" . $this->opts['id'] . " = window.intlTelInput(el, " . $tel_params . "); jQuery(el).on('keyup', function(e) { const check = iti_" . $this->opts['id'] . ".isValidNumber() ? 1 : 0; el.dataset.validnumber = check; el.dataset.fullnumber = iti_" . $this->opts['id'] . ".getNumber(intlTelInputUtils.numberFormat.E164); }); }</script>";
                }
                else
                {
                    wp_add_inline_script('rm_mobile_script',"if (typeof telDuplicate_" . $this->opts['id'] . " === 'undefined') { telDuplicate_" . $this->opts['id'] . " = true; const el = document.querySelector('#" . $this->opts['id'] . "'); var iti_" . $this->opts['id'] . " = window.intlTelInput(el, {$tel_params}); jQuery(el).on('keyup', function(e) { const check = iti_" . $this->opts['id'] . ".isValidNumber() ? 1 : 0; el.dataset.validnumber = check; el.dataset.fullnumber = iti_" . $this->opts['id'] . ".getNumber(intlTelInputUtils.numberFormat.E164); }); }");
    ...
    public function create_pgavatar_field() {
        $this->field_options->field_user_profile = 'define_new_user_meta';
        $this->field_options->field_meta_add = 'pm_user_avatar';
        return $this->create_image_field();
    }
    ...
    public function create_bdate_field() {
        if(is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev'])) {
            $current_user = wp_get_current_user();
            $user_bday_info = get_user_meta($current_user->ID, $this->get_user_meta_key(), true);
            $this->opts['value'] = empty($user_bday_info) ? $this->opts['value'] : $user_bday_info;
        }
    ...
    public function create_gender_field() {
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page) {
            ...
            $current_user = wp_get_current_user();
            $user_gender_info = get_user_meta($current_user->ID,$this->get_user_meta_key(), true);
            $this->opts['value'] = empty($user_gender_info) ? $this->opts['value'] : $user_gender_info;
        }
    ...
    public function create_radio_field() {
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page) {
            ...
            $this->opts['value'] = empty($radio_info) ? $this->opts['value'] : $radio_info;
        }
    ...
    public function create_checkbox_field() {
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page ) {
            ...
            $this->opts['value'] = empty($checkbox_info) ? $this->opts['value'] : $checkbox_info;
        }
    ...
    public function create_divider_field() {
        $class = isset($this->opts['class']) ? $this->opts['class'] . ' rm-full-width' : 'rm-full-width';
    ...
    public function create_time_field() {
        ...
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && ($this->db_field->field_show_on_user_page)) {
            $current_user = wp_get_current_user();
            $time_info = get_user_meta($current_user->ID, $this->get_user_meta_key(), true);
            $this->opts['value'] = empty($time_info) ? $this->opts['value'] : $time_info;
        }
    ...
    public function create_rating_field() {
        if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page) {
            $current_user = wp_get_current_user();
            $user_rating = get_user_meta($current_user->ID,$this->get_user_meta_key(), true);
            $this->opts['value'] = empty($user_rating) ? $this->opts['value'] : $user_rating;
        }
        ...
        if ($this->db_field->is_field_primary && !$this->prevent_value_update) {
            ...
            else
            {
                ...
                if (isset($field_options->en_confirm_email) && $field_options->en_confirm_email) {
                    ...
                    array_push($fields_array, new RM_Frontend_Field_Base($this->db_field->field_id,$this->db_field->field_type, $this->field_name, $this->db_field->field_label, $this->opts, $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts));
                    
                    $cp_opts['name']="email_confirmation";
                    array_push($fields_array, new RM_Frontend_Field_Base($this->db_field->field_id,$this->db_field->field_type, $this->field_name, $this->db_field->field_label, $cp_opts, $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts));
                    
                    return $fields_array;
                }
            }
        } else {
            if (is_user_logged_in() && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page) {
                $current_user = wp_get_current_user();
                $user_emailinfo = get_user_meta($current_user->ID, $this->get_user_meta_key(), true);
                $this->opts['value'] = empty($user_emailinfo) ? $this->opts['value'] : $user_emailinfo;
            }
        }
    ...
    public function create_wcbilling_field() {
        return new RM_Frontend_Field_WCAddress($this->db_field->field_id, $this->db_field->field_type, $this->field_name, $this->db_field->field_label, $this->opts, $this->db_field->page_no,'billing', $this->db_field->is_field_primary, $this->x_opts);
    }
    
    public function create_wcshipping_field() {
        return new RM_Frontend_Field_WCAddress($this->db_field->field_id, $this->db_field->field_type, $this->field_name, $this->db_field->field_label, $this->opts, $this->db_field->page_no,'shipping', $this->db_field->is_field_primary, $this->x_opts);
    }
    ...
    public function create_richtext_field() {
        return new RM_Frontend_Field_Visible_Only($this->db_field->field_id, $this->db_field->field_type, $this->field_name, $this->db_field->field_label, $this->opts, wpautop($this->db_field->field_value), $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts);
    }
    ...
    public function create_youtubev_widget() {
        ...
        $src = "https://www.youtube.com/embed/" . $video_id . "?enablejsapi=1".($this->db_field->page_no==1?'&autoplay='. $this->field_options->yt_auto_play:'');
        $src .= $this->field_options->yt_repeat ? "&playlist=" . $video_id . "&loop=1" : '';
        $src .= empty($this->field_options->yt_related_videos) ? '&rel=0' : '';
        $allow_autoplay = $this->field_options->yt_auto_play==1?"class='allow-autoplay' id='video_".$this->db_field->form_id."_".$this->db_field->page_no."'":"";
        
        $iframe = "<div class='rmrow'><iframe width='" . $width . "' height='" . $height . "' src='" . $src . "' frameborder='0' ".($this->field_options->yt_auto_play==1 && $this->db_field->page_no==1?"allow='autoplay'":"")." allowfullscreen ".$allow_autoplay."></iframe></div>";
    ...
    public function create_username_field() {
        ...
        return new RM_Frontend_Field_Base($this->db_field->field_id, $this->db_field->field_type, "username", $this->db_field->field_label, $this->opts, $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts);
    }
    ...
    public function create_userpassword_field() {
        ...
        $fields_array = array();
        if ($this->gopts->get_value_of('enable_custom_pw_rests') == 'yes') {
            ...
            $this->opts['validation'] = new Validation_RegExp('/' . $patt_regex . '/', $error_str);
            $this->opts['title']= $error_str;
            $this->opts['pattern'] = $patt_regex;
            ...
            if(is_array($pw_rests->selected_rules)){
                if (in_array('PWR_MINLEN', $pw_rests->selected_rules) && isset($pw_rests->min_len) && $pw_rests->min_len)
                    $this->opts["minlength"] = $pw_rests->min_len;

                if (in_array('PWR_MAXLEN', $pw_rests->selected_rules) && isset($pw_rests->max_len) && $pw_rests->max_len)
                    $this->opts["maxlength"] = $pw_rests->max_len;
            }   

            array_push($fields_array, new RM_Frontend_Field_Base($this->db_field->field_id, $this->db_field->field_type, "pwd", $this->db_field->field_label, $this->opts, $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts));
        }
        else {
        ...
        if (empty($pw_rests) && $field_options->en_pass_strength) {
        ...
        if ($field_options->en_confirm_pwd) {
            array_push($fields_array, new RM_Frontend_Field_Base($this->db_field->field_id, $this->db_field->field_type, "password_confirmation", $this->db_field->field_label, $cp_opts, $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts));
        }
    ...
    public function create_mapv_field(){
        ...
        if(!empty($address) && !empty($gmap_api_key)){
            if(!wp_script_is('google_map_api', 'enqueued')) {
                wp_enqueue_script ('google_map_api', 'https://maps.googleapis.com/maps/api/js?key='.$gmap_api_key.'&libraries=places');
    ...
    public function create_privacy_field(){
        ...
        $content = str_replace('{{privacy_policy}}',"<a target='_blank' href='$url'>".__('Privacy Policy','registrationmagic-addon')."</a>",$content);
        $icon='';
        $field_label='';
        ...
        if(!empty($checkbox)) {
            $required_text = __('This field is required', 'registrationmagic-addon');
    ...
    public function create_activeuser_field() {
        ...
        if( function_exists('rm_active_user_list')) {
            if (is_user_logged_in() && !$this->prevent_value_update && !isset($_GET['form_prev']) && $this->db_field->field_show_on_user_page) {
                $current_user = wp_get_current_user();
                $select_field = rm_active_user_list();
                $this->opts['value'] = empty($select_field) ? $this->opts['value'] : $select_field;
            }
            return new RM_Frontend_Field_Select_Addon($this->db_field->field_id, $this->db_field->field_label, $this->field_name, $this->opts, $this->db_field->field_value, $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts);
        }else{
            $html = '';
            return new RM_Frontend_Field_Visible_Only($this->db_field->field_id,'HTMLCustomized',$this->field_name, $this->db_field->field_label, $this->opts,$html, $this->db_field->page_no, $this->db_field->is_field_primary, $this->x_opts);
        }
