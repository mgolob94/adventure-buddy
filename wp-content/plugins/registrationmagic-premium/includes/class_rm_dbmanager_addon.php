...
class RM_DBManager_Addon
{
    ...
    {
        if (!isset(self::$instance) && !( self::$instance instanceof RM_DBManager_Addon ))
        {
            self::$instance = new RM_DBManager_Addon;
        }
    ...
    public static function update_unread_status_submission($sub_id, $status)
    {
        ...
        $result = $wpdb->update($table_name, array('is_read' => $status), array('submission_id' => $sub_id), array('%d'), array('%d'));
    ...
    public static function update_read_status_all_submissions($form_id, $status)
    {
        ...
        $result = $wpdb->update($table_name, array('is_read' => $status), array('form_id' => $form_id), array('%d'), array('%d'));
    ...
    public static function set_form_page_order($form_id, $order_list)
    {
        ...
        {        
            ...
            $x = (object) array('ordered_form_pages' => $order_list);
    ...
    /**
     * This function get all the submission Id's for a specific custom status
     *
     * @param   $status_index   int         id of the index for which the submission is searched
     * @param   $form_id        int         id of the form to be searched
     *
     * @return  Array   array of all the submission ids for the field value
     */
    ...
    public static function search_submissions_for_custom_status($status_index,$form_id)
    {
        ...
        $status_index= explode(',', $status_index);
   ...
   public static function get_all_form_attachments($form_id,$selection='value')
    {
        ...
        $image_ids=  self::get('FIELDS', array('field_type' => 'Image', 'form_id' => $form_id), array('%s','%d'), 'col', 0, 99999, 'field_id', null, false);
        ...
        $field_ids=array_merge($field_ids,$image_ids);
        ...
        if ($field_ids)
        {
            $qry = "SELECT $selection FROM $table_name WHERE ";

            $field_ids= implode(',',$field_ids);

            $qry .= " `field_id` in ($field_ids) AND value is not null ORDER BY `$unique_id_name`";
            ...
            if(is_array($filter_tags)){ 
                foreach($filter_tags as $filter_tag):
                    switch(strtolower($filter_tag)){
                        ...
                        case strtolower(trim(RM_UI_Strings::get("LABEL_NO_ATTACHMENT"))):
                        $submission_ids= self::get_all_form_attachments($form_id,' distinct submission_id ');
                        if($submission_ids):
                        ...
                        case strtolower(trim(RM_UI_Strings::get("LABEL_HAVE_NOTE"))):
                        ...
                        case strtolower(trim(RM_UI_Strings::get("LABEL_PAYMENT_PENDING"))):
                        ...
                        case strtolower(trim(RM_UI_Strings::get("LABEL_PENDING_OFFLINE_PAYMENTS"))):
                            $submission_ids= self::get_submissions_by_payment_type($form_id,"'offline'");
                            $pending_submission_ids= self::get_submissions_payment_status($form_id,"'pending','Pending','".__('Pending', 'registrationmagic-addon')."','".strtolower(__('Pending', 'registrationmagic-addon'))."'");
                        ...
                        case strtolower(trim(RM_UI_Strings::get("LABEL_PAYMENT_RECEIVED"))):
                        ...
                        case strtolower(trim(RM_UI_Strings::get("LABEL_READ"))):
                        ...
                        case strtolower(trim(RM_UI_Strings::get("LABEL_UNREAD"))):
                         ...
                         case strtolower(trim(RM_UI_Strings::get("LABEL_BLOCKED"))):
    ...
    public static function get_submissions_with_note($form_id){
        ...
        $table_notes = RM_Table_Tech::get_table_name_for('NOTES');
        $table_submissions = RM_Table_Tech::get_table_name_for('SUBMISSIONS');
        
        $qry = "SELECT s.submission_id from $table_submissions s INNER JOIN  $table_notes n on s.submission_id=n.submission_id and s.form_id=%d";
    ...
    public static function get_submissions_by_payment_type($form_id,$type){
        ...
        $table_payment_logs = RM_Table_Tech::get_table_name_for('PAYPAL_LOGS');
        $qry = "SELECT submission_id from $table_payment_logs where form_id=%d and pay_proc in ($type) "; 
        $submission_ids=$wpdb->get_col($wpdb->prepare($qry,$form_id));
     ...
     * @global object $wpdb
     * @param type $form_id
     * @param type $type (1 for readed and 0 for unreaded submission)
     * @return type int
     */
     public static function get_submission_read_count($form_id,$type,$count= true){
        ...
        if($count):
            $qry = "SELECT COUNT(*) as count FROM $table_name WHERE is_read=%d and form_id=%d";
            $count= $wpdb->get_var($wpdb->prepare($qry,$type,$form_id)); 
            return $count;
        else:
            $qry = "SELECT submission_id FROM $table_name WHERE is_read=%d and form_id=%d";
            $submissions_ids= $wpdb->get_col($wpdb->prepare($qry,$type,$form_id));    
     ...
     * 
     * @global object $wpdb
     * @param type $type (1 for uneaded and 2 for submission within 1 hour and 3 for submission within 24 hour)
     * @return type int
     */
    public static function get_all_submission_read_count($type){
        ...
        if ($type == 1){
            $qry = "SELECT COUNT(*) as count FROM $table_name WHERE is_read=%d";
            $count= $wpdb->get_var($wpdb->prepare($qry,0));
        }else if ($type == 2){
            $t=time()-3600;
            $date = date('Y-m-d H:i:s', $t);
            $table_name = $wpdb->prefix . 'rm_submissions';
            $qry = $wpdb->prepare("SELECT COUNT(*) as count FROM $table_name WHERE submitted_on >= %s", $date);
            $count = $wpdb->get_var($qry);
        }else if ($type == 3){
            $t = time() - 86400;
            $date = date('Y-m-d H:i:s', $t);
     ...
     public static function get_blocked_submission($form_id){
     	global $wpdb;
     	$table_name = RM_Table_Tech::get_table_name_for('SUBMISSIONS');
     	$query=  "SELECT submission_id FROM $table_name WHERE form_id=%d";
     	$all_submissions= $wpdb->get_col($wpdb->prepare($query,$form_id));  
        $submission_ids= array();
        $submission_model= new RM_Submissions();
        if(is_array($all_submissions) && count($all_submissions)>0):
            foreach($all_submissions as $submission):
                $submission_model->load_from_db($submission);
                if($submission_model->is_blocked())
   ...
   public static function get_latest_submission_for_user($user_email, $form_ids = array()) {
    ...
    public static function get_recent_submissions_for_user($user_email,$form_ids = array()){
        ...
        if(count($form_ids) !== 0){
           ...
           $specifier_str= implode(', ',$data_specifiers); 
           $query= "SELECT * FROM $table_name WHERE $unique_id_name IN ("
                . "SELECT $unique_id_name FROM `$table_name` where `user_email` =%s "
                ." AND `form_id` IN ($specifier_str) and child_id=0 )"
                . "ORDER BY $unique_id_name DESC";
           $results = $wpdb->get_results($wpdb->prepare($query,array_merge(array($user_email),$form_ids)));
        }  
        else{
    ...
    public static function get_edd_user_details($user_email){
        ...
        $query= "SELECT * FROM ".$wpdb->prefix."edd_customers WHERE email =%s";
        $results = $wpdb->get_row($wpdb->prepare($query,$user_email));
    ...
    public static function get_recent_edd_orders_for_user($payment_ids){
        ...
        $payment_id_arr = explode(',', $payment_ids);
        $stringPlaceholders = array_fill(0, count($payment_id_arr), '%s');
        $placeholdersForPaymentIds = implode(',', $stringPlaceholders);
        
        $query= "SELECT * FROM ".$wpdb->prefix."posts WHERE ID IN ($placeholdersForPaymentIds) ORDER BY `ID` DESC";
        $results = $wpdb->get_results($wpdb->prepare($query,$payment_id_arr));
        //echo '<pre>';print_r($payment_details);die;
        if ($results === NULL || count($results) === 0){
            return false;
        }else{
            $payment_details = array();
            foreach ($results as $result){
                $status = ($result->post_status=='publish')?'Complete': ucfirst($result->post_status);
                $payment_meta_arr = get_post_meta($result->ID,'_edd_payment_meta',true);
                $payment_details[] = array('ID'=>$result->ID,'date'=>$result->post_date,'amount'=>get_post_meta($result->ID,'_edd_payment_total',true),'currency'=>$payment_meta_arr['currency'],'status'=>$status);
            }
   ...
   public static function get_oldest_submission_from_group($submission_id) {
    ...
    public static function run_query($query, $result_type = 'results') {
      ...
      {          
        ...
        $table_name_note = RM_Table_Tech::get_table_name_for('NOTES');
        $table_name_submission = RM_Table_Tech::get_table_name_for('SUBMISSIONS');
        
        $sub_ids = $wpdb->get_col($wpdb->prepare("SELECT `submission_id` FROM $table_name_submission WHERE `last_child` = %d",$sub_id));
        
        if(is_admin())
            $result = $wpdb->get_results("SELECT * FROM $table_name_note WHERE `submission_id` IN (".implode(',', $sub_ids).") ORDER BY note_id DESC");
        else
            $result = $wpdb->get_results("SELECT * FROM $table_name_note WHERE `status` = 'publish' AND `submission_id` IN (".implode(',', $sub_ids).") ORDER BY note_id DESC");
      ...
      public static function remove_custom_status_from_submissions($form_id,$status_index){
          ...
          $status_index= absint($status_index);
          $result = $wpdb->query($wpdb->prepare("DELETE FROM `$table_name` where `form_id` = %d and `status_index`=%d",$form_id,$status_index));
      }
      ...
      public static function is_allowed_by_custom_status($email,$form_id){
          ...
          if(empty($form_id))
                  return false;
          $submission_table = RM_Table_Tech::get_table_name_for('SUBMISSIONS');
          $cs_table = RM_Table_Tech::get_table_name_for('CUSTOM_STATUS');
          
          $form_model= new RM_Forms;
          $form_model->load_from_db($form_id);
          $form_options= $form_model->get_form_options();
          $auto_exp= $form_model->get_form_should_auto_expire();
         
          if(!empty($auto_exp) && $form_options->form_expired_by=='status'){
              $statuses= $form_options->form_limit_by_cs;
              if(!empty($statuses)){
                  foreach($statuses as $status){
                      if($status==='')
                          continue;
                      $st_arr= explode(':', $status);
                      ...
                      $count= $wpdb->get_var($wpdb->prepare("SELECT count(*) FROM $submission_table as sub left join $cs_table as cs on sub.submission_id=cs.submission_id where user_email=%s and sub.form_id=%d and status_index=%d",$email,$form_id,$status));
                      if($count==0)
                       return false;
                  }
                  return true;
...
public static function insert_login_log($args){
    ...
    public static function delete_sent_emails($email){
        ...
        $wpdb->query("DELETE FROM `$table_name` WHERE `to` = '".$email."'");
    }
    
    public static function delete_all_tables(){echo 'dddd';die;
        global $wpdb;
        ...
        $table_name = RM_Table_Tech::get_table_name_for('RES');
        $wpdb->query("DROP table `$table_name`");
    ...
    {
        global $wpdb;
        $count= -14;    //2 for default forms and 12 for fields
    ...
    public static function get_mailpoet_fields($form_id)
    {
        global $wpdb;
        $table_name = $wpdb->prefix.'mailpoet_forms';
        $result = maybe_unserialize($wpdb->get_var($wpdb->prepare("Select `body` FROM `$table_name` where `id` = %d",$form_id)));
        if (empty($result))
        {
            return false;
        }
    ...
    public static function get_mailpoet_lists($form_id)
    {
        global $wpdb;
        $table_name = $wpdb->prefix.'mailpoet_forms';
        $result = maybe_unserialize($wpdb->get_var($wpdb->prepare("Select `settings` FROM `$table_name` where `id` = %d",$form_id)));
    ...
    public static function insert_tabs_row($array_attributes){
        global $wpdb;
        $table_name = RM_Table_Tech::get_table_name_for('TABS');
        $db_format = new RM_DB_FORMAT;
        $result = $wpdb->insert($table_name, $array_attributes);
        if ($result !== false)
            return $wpdb->insert_id;
        else
            return false;
    }
    public static function get_tabs_row($tab_id){
        global $wpdb;
        $table_name = RM_Table_Tech::get_table_name_for('TABS');
        $res = $wpdb->get_row($wpdb->prepare("SELECT * from `$table_name` where `tab_id` = %d", $tab_id));
        return $res;
    }

    public static function update_tabs_row($array_attributes,$unique_identifier){
        global $wpdb;
        $table_name = RM_Table_Tech::get_table_name_for('TABS');
        $res = $wpdb->update($table_name, $array_attributes, $unique_identifier);
    ...
    public static function remove_tabs($tab_id){
        global $wpdb;
        $table_name = RM_Table_Tech::get_table_name_for('TABS');
        $tab_id= absint($tab_id);

        $result = $wpdb->query($wpdb->prepare("DELETE FROM `$table_name` where `tab_id` = %d",$tab_id));
    }
    public static function get_all_form_reports_attachments($form_id,  $parameter, $selection='value')
    {
        ...
        $table_name = RM_Table_Tech::get_table_name_for('SUBMISSION_FIELDS');
        $submission_table_name = RM_Table_Tech::get_table_name_for('SUBMISSIONS');
        $unique_id_name = RM_Table_Tech::get_unique_id_name('SUBMISSION_FIELDS');
        ...
        {
            $field_ids= implode(',',$field_ids);
            $interval_string = "BETWEEN '" .date('Y-m-d',strtotime($parameter->start_date)). "' AND '".date('Y-m-d',strtotime($parameter->end_date))."'  ORDER BY `submission_id` DESC ";
            $submission_qry = "SELECT submission_id FROM `$submission_table_name` WHERE CAST(submitted_on AS date) $interval_string";
            $submissions = $wpdb->get_results($submission_qry);
            $results= array();
            if(!empty($submissions)){
                foreach($submissions as $submission){
                    $sub_id = $submission->submission_id;
                    $qry = "SELECT $selection FROM $table_name WHERE ";
                    $qry .= " `field_id` in ($field_ids) AND value is not null AND `submission_id` in ($sub_id) ORDER BY `$unique_id_name`";
    ...
    public static function add_payments_filter_queries($form_id, $filters, $qry) {
        ...
        if (!empty($filters['filter_tags'])) {
            ...
            if (is_array($filter_tags)) {
                foreach ($filter_tags as $filter_tag):
                    switch (strtolower($filter_tag)) {
                        case 'canceled':
                            $submission_ids = self::get_submissions_payment_status($form_id, "'canceled'");
                            if (count($submission_ids)):
                                $records = array_merge($records, $submission_ids);
                            endif;
                            ...
                            break;
                        case 'refunded':
                            $submission_ids = self::get_submissions_payment_status($form_id, "'refunded'");
                            if (count($submission_ids)):
                                $records = array_merge($records, $submission_ids);
                            endif;
                ...
                if (empty($records) && empty($excluded_records)):
                    $qry .= " and $table_name.submission_id in (-1) ";
                elseif (count($excluded_records)):
                    $submission_ids = implode(',', $excluded_records);
                    $qry .= " and $table_name.submission_id not in ($submission_ids)";
                endif;
                ...
                if (count($records)) {
                    ...
                    if (count($excluded_records))
                        $qry .= " OR $table_name.submission_id in ($submission_ids)";
                    else
                        $qry .= " AND $table_name.submission_id in ($submission_ids)";
                }
            }
        }
