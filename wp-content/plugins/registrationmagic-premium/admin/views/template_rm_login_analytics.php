...
<div class="rmagic">
    <div class="operationsbar">
        <div class="rmtitle"><?php echo _e('Login Timeline', 'registrationmagic-addon'); ?></div>
        <div class="nav">
            <ul>
    ...
    <div class="rm-login-timeline rm-dbfl">
        ...
        <div  class="rm-advanced-timeline rm-dbfl"><a href="?page=rm_login_advanced"><?php _e('Advanced View', 'registrationmagic-addon'); ?></a></div>
            ...
            <tbody>
                ...
                if(isset($data->login_logs)){
                    if (!empty($data->login_logs) && (is_array($data->login_logs) || is_object($data->login_logs))){
                                    ...
                                    <div class="rm-modal-container">
                                        <div class="rm-login-user-details-dialog">
                                            ...
                                            <div class="rm-login-details-rf">
                                                ...
                                                <div class="rm-login-user-info"><?php _e('IP Ban', 'registrationmagic-addon'); ?><span><?php echo ($login_log->ban==1)?'YES':'NO' ?></span></div>
                                                <div class="rm-login-user-info"><?php _e('Final Result', 'registrationmagic-addon'); ?><span><?php echo ($login_log->status==1)?'Login Successful':'Login Denied' ?></span></div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                            ...
                            <div id="rm-send-email-<?php echo $login_log->id; ?>" class="rm-login-user-details rm-send-email" style="display:none">
                                ...
                                <div class="rm-modal-wrap">
                                    ...
                                    <div class="rm-modal-container rm-dbfl">
                                        ...
                                        <div class="rm-dbfl rm-email-box-row"><span class="rm-field-lable rm-difl"><?php _e("Subject", 'registrationmagic-addon'); ?>:</span><span class="rm-field-input rm-difl"><input name="rm-user-subject" value="RM LOGIN EMAIL" class="rm-user-subject-<?php echo $login_log->id; ?>" placeholder="Type your subject here"/></span></div>
                                        <div class="rm-dbfl rm-email-box-row"><span class="rm-field-lable rm-difl"><?php _e("Message", 'registrationmagic-addon'); ?>:</span><span class="rm-field-input rm-difl"><textarea name="rm-user-message" class="rm-user-message-<?php echo $login_log->id; ?>" placeholder="Type your message here"></textarea></span></div>
                                    </div>
                        ...
                        echo '<div class="rmnotice">'.sprintf(__('Not enough data. Come back later to check login activity. <a target="_blank" href="%s">More Info</a>', 'registrationmagic-addon'),'https://registrationmagic.com/wordpress-user-login-plugin-guide/#login-analytics').'</div>';
                    }
                }
                ?>
            </tbody>
    ...
    <div class="rm-grid-section dbfl" id="rm_tour_timewise_stats">
        <div class="rm-grid-section-title dbfl rm-box-title"><?php _e('Login Failures vs. Login Success Over Time (Line Graph)', 'registrationmagic-addon'); ?></div>
...
function rm_login_unblock_ip(user_ip){
    ...
    jQuery.post(ajaxurl, data, function(response) {
        if(response=='success'){
            jQuery('.rm_status_update_body').html('IP '+user_ip+' was unblocked successfully.')
            jQuery('#rm_status_update_model').addClass('rm-modal-show');
             setTimeout(function(){
             jQuery('#rm_status_update_model').removeClass('rm-modal-show');
             location.reload(true);
...
function rm_login_send_email(user_email,log_id){
    ...
    jQuery.post(ajaxurl, data, function(response) {
        ...
        if(response=='success'){
            jQuery('.rm_status_update_body').html('Mail Sent successfully.')
            jQuery('#rm_status_update_model').addClass('rm-modal-show');
            setTimeout(function(){
                  jQuery('#rm_status_update_model').removeClass('rm-modal-show');
                  location.reload(true);
