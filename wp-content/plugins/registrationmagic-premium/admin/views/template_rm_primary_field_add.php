...
if(strtolower($data->model->field_type)=="username" || strtolower($data->selected_field)=='username'){
    $form->addElement(new Element_Textarea("<b>".__('Username exists error','registrationmagic-addon')."</b>", "user_exists_error", array("class" => "", "value" =>$data->model->field_options->user_exists_error , "longDesc" => sprintf(__('Contents of the error message user sees when trying to register with existing or blacklisted/ reserved username. You can manage blacklisted/ reserved usernames in Global Settings → Security. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-addon'),'https://registrationmagic.com/knowledgebase/username-field-settings/#htexisterror'))));
    $form->addElement(new Element_Checkbox("<b>".__('Allowed Characters','registrationmagic-addon')."</b>", "username_characters", array('alphabets' => __('Alphabets','registrationmagic-addon'),'numbers'=>__('Numbers','registrationmagic-addon'),'underscores'=>__('Underscores','registrationmagic-addon'),'periods'=>__('Periods','registrationmagic-addon')), array("class" => "rm_field_multiline rm_input_type", "value" => $data->model->field_options->username_characters, "longDesc"=>sprintf(__('Select the type of characters accepted for usernames. If none selected, All the characters would be allowed. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-addon'),'https://registrationmagic.com/knowledgebase/username-field-settings/#htallowedchars'))));
    $form->addElement(new Element_Textarea("<b>".__('Invalid username format error','registrationmagic-addon')."</b>", "invalid_username_format", array("class" => "", "value" => $data->model->field_options->invalid_username_format, "longDesc" => sprintf(__('Contents of the error message user sees when trying to enter username with characters not allowed. Use code {{allowed_characters}} to insert allowed characters dynamically. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-addon'),'https://registrationmagic.com/knowledgebase/username-field-settings/#htinvalidformat'))));
}
if(strtolower($data->model->field_type)=="userpassword" || strtolower($data->selected_field)=='userpassword'){
    $form->addElement(new Element_Checkbox("<b>".__('Display password confirmation field','registrationmagic-addon')."</b>", "en_confirm_pwd", array('1'=>''), array("class" => "rm_field_multiline rm_input_type en_confirm_pwd", "value" => $data->model->field_options->en_confirm_pwd, "longDesc"=>sprintf(__('Display a password confirmation field below the password field. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-addon'),'https://registrationmagic.com/knowledgebase/password-field-settings/#htdisplayconfirmation'))));
        $form->addElement(new Element_HTML('<div class="childfieldsrow" id="confirm_pwd_options">'));
             $form->addElement(new Element_Textarea("<b>".__('Password match error','registrationmagic-addon')."</b>", "pass_mismatch_err", array("class" => "", "value" =>$data->model->field_options->pass_mismatch_err , "longDesc" => sprintf(__('Contents of the error message user sees when the password entered in password confirmation field does not matches. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-addon'),'https://registrationmagic.com/knowledgebase/password-field-settings/#htmatcherror'))));
        $form->addElement(new Element_HTML('</div>'));
    $gopts= new RM_Options();
    $pw_rests= $gopts->get_value_of('enable_custom_pw_rests');
    $disable= '';
    if(!empty($pw_rests)){
       $disable= 'rm_disabled'; 
    }
    $gopts= new RM_Options();
    $enable= $gopts->get_value_of('enable_custom_pw_rests');

    if($enable=='yes'){
       $form->addElement(new Element_HTML('<div class="rm-disable">'));  
    }
      
        $form->addElement(new Element_Checkbox("<b>".__('Display password strength meter','registrationmagic-addon')."</b>", "en_pass_strength", array('1'=>''), array("class" => "rm_field_multiline rm_input_type en_pass_strength ", "value" => $data->model->field_options->en_pass_strength, "longDesc"=>sprintf(__('Displays a live password strength meter to provide feedback to the user about the strength of the password they are entering. For advance password rules, checkout Global Settings → Security. <a target="_blank" class="rm-more" href="%s">More</a>','registrationmagic-addon'),'https://registrationmagic.com/knowledgebase/password-field-settings/#htstrengthmeter'))));   
        $form->addElement(new Element_HTML("<div class='childfieldsrow' id='pwd_strength_options'>"));
            $form->addElement(new Element_Textbox("<b>".__('Short','registrationmagic-addon')."</b>", "pwd_short_msg", array("class" => "rm_static_field", "value" => $data->model->field_options->pwd_short_msg, "longDesc"=>'')));
            $form->addElement(new Element_Textbox("<b>".__('Weak','registrationmagic-addon')."</b>", "pwd_weak_msg", array("class" => "rm_static_field", "value" => $data->model->field_options->pwd_weak_msg, "longDesc"=>'')));
            $form->addElement(new Element_Textbox("<b>".__('Medium','registrationmagic-addon')."</b>", "pwd_medium_msg", array("class" => "rm_static_field", "value" => $data->model->field_options->pwd_medium_msg, "longDesc"=>'')));
            $form->addElement(new Element_Textbox("<b>".__('Strong','registrationmagic-addon')."</b>", "pwd_strong_msg", array("class" => "rm_static_field", "value" => $data->model->field_options->pwd_strong_msg, "longDesc"=>'')));
        $form->addElement(new Element_HTML('</div>'));
    if($enable=='yes'){
       $form->addElement(new Element_HTML('</div>'));  
    }    
...
if(strtolower($data->model->field_type)=="email" && $data->model->is_field_primary == 1) {
    $form->addElement(new Element_Checkbox("<b>".__('Display email confirmation field','registrationmagic-addon')."</b>", "en_confirm_email", array('1'=>''), array("class" => "rm_field_multiline rm_input_type en_confirm_email", "value" => $data->model->field_options->en_confirm_email, "longDesc"=>__('Display an email confirmation field below the email field.','registrationmagic-addon'))));
    $form->addElement(new Element_HTML('<div class="childfieldsrow" id="confirm_pwd_options">'));
    $form->addElement(new Element_Textarea("<b>".__('Email mismatch error message','registrationmagic-addon')."</b>", "email_mismatch_err", array("class" => "", "value" =>$data->model->field_options->email_mismatch_err , "longDesc" => __('Contents of the error message user sees when the email entered in email confirmation field does not match.','registrationmagic-addon'))));
    $form->addElement(new Element_HTML('</div>'));
}
