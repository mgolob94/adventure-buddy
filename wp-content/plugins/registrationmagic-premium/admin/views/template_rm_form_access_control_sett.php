...
$domain_cb_value = isset($actrl->domain) ? 1 : 0;
$domain =  isset($actrl->domain) ? $actrl->domain : '';
...
<div class="rmagic">
    ...
    <div class="rmcontent">
        ...
        $form->addElement(new Element_Checkbox("<b>" . __('Domain based form access control','registrationmagic-addon') . "</b>", "form_actrl_domain_cb", array(1=>''), array("id" => "id_form_actrl_domain_cb", "value" =>$domain_cb_value, "onclick" => "actrl_domain_click_handler(this)", "longDesc" =>__('Only users with specified email domain will be able to submit the form','registrationmagic-addon'))));
            if($domain_cb_value){
                $form->addElement(new Element_HTML("<div id='form_actrl_domain_container' class='childfieldsrow' style='border: 1px solid transparent;'>"));
            } else{
                $form->addElement(new Element_HTML("<div id='form_actrl_domain_container' class='childfieldsrow' style='display:none;border: 1px solid transparent;'>"));
            }
                $form->addElement(new Element_Textbox("<b>" . __('Domain','registrationmagic-addon') . "</b>", "form_actrl_domain", array("value" => $domain, "longDesc" => __('Only users with email on this domain will be allowed to submit form/ register.','registrationmagic-addon'))));
            $form->addElement(new Element_HTML("</div>"));
...
function actrl_domain_click_handler(obj){
    obj = jQuery(obj);
    if(obj.is(':checked')){
        jQuery('#form_actrl_domain_container').slideDown();
        return;
    }
    jQuery('#form_actrl_domain_container').slideUp();
};
...
function prevent_save(event){
    ...
    var max_date=new Date(jQuery("#form_actrl_date_ul_date").val());
        ...
        if(max_date<=min_date)
               {
                   jQuery('#date_error').html("<?php _e('Invalid Date Range','registrationmagic-addon'); ?>");
   ...
   if( jQuery("#id_form_actrl_date_type-0").is(":checked") )
    ...
    var max_date= parseInt(jQuery("#form_actrl_date_ul_diff").val());
        if(max_date<=min_date)
               {
                   jQuery('#date_limit_error').html("<?php _e('Invalid Age Limit Range','registrationmagic-addon'); ?>");
