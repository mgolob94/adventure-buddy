...
if($data->type=='message')
{
  ...
  $form->addElement(new Element_HTMLL('&#8592; &nbsp; '.__('Cancel','registrationmagic-addon'), '?page=rm_submission_view&rm_submission_id=' . $data->submission_id, array('class' => 'cancel')));
  $form->addElement(new Element_Button(RM_UI_Strings::get('LABEL_SEND'), "submit", array("id" => "rm_submit_btn", "class" => "rm_btn", "name" => "submit")));
    $form->render();    
        ...
        $form->addElement(new Element_HTMLL('&#8592; &nbsp; '.__('Cancel','registrationmagic-addon'), '?page='.$data->redirectpage.'&rm_submission_id=' . $data->submission_id, array('class' => 'cancel')));
