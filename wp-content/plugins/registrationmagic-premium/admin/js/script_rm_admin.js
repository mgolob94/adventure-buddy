 jQuery(document).ready(function(){
    ...
    jQuery("#rm_dpx_conf_notice button").click(function(){
    var data = {
                action: 'rm_admin_disable_notice',
    ...
    jQuery(".rm_dismis_cc_banner .rm_dismiss").click(function(){
        jQuery('.rm_dismis_cc_banner').slideUp();
        var data = {
                action: 'rm_admin_disable_notice',
                'rm_sec_nonce': rm_admin_vars.nonce,
                disable_cc: 1
...
(function (RM_jQ) {
    ...
    RM_jQ(document).ready(function () {
        ...
        RM_jQ("#rm_field_is_unique-0").change(function () {
           if(RM_jQ(this).attr('checked'))
               RM_jQ("#rm_unique_err_msg").show();
           else
               RM_jQ("#rm_unique_err_msg").hide();
...
//        RM_jQ("#scroll").hide();
        ...
        RM_jQ("#rm_rating_config_container").hide();
        ...
        switch (field_type) {
            ...
            case 'WCBillingPhone' :
                var object = RM_jQ(".rm_field_value, .rm_textarea_type, .rm_options_type_fields, #rm_field_is_read_only-0, #rm_field_is_editable-0, #rm_field_max_length, #rm_field_min_length");
                break;
                
            case 'Privacy':
                var object = RM_jQ(".rm_field_value, .rm_textarea_type, .rm_options_type_fields, #rm_field_is_read_only-0, #rm_field_max_length, #rm_field_min_length, #rm_field_is_required-0, #rm_field_is_editable-0, #rm_sub_heading");
                RM_jQ('#rm_field_placeholder').parents(".rmrow").hide();
            ...
            case 'Hidden':
                var object = RM_jQ(".rm_field_value, .rm_textarea_type, .rm_options_type_fields, #rm_field_is_read_only-0, #rm_field_placeholder, #rm_field_helptext, #rm_field_class, #rm_field_max_length, #rm_field_min_length, #rm_field_is_required-0, #rm_field_is_editable-0").not("#rm_field_default_value");
            ...
            case 'Custom' :
                ...
                break;
            case 'HTMLP' :
                var object = RM_jQ(".rm_input_type, .rm_field_value, #rm_field_is_editable-0").not("#rm_field_value_paragraph");
                ...
                break;
            case 'Shortcode' :
                var object = RM_jQ(".rm_input_type, .rm_field_value, #rm_field_is_editable-0").not("#rm_field_value_shortcode");
                var val_field = RM_jQ("#rm_field_value_shortcode");
                RM_jQ(".rm_sub_heading").hide();
                RM_jQ("#rm_icon_setting_container,#rm_icon_field_settings_header").hide();
                break;
            case 'HTMLH' :
                var object = RM_jQ(".rm_input_type, .rm_field_value, #rm_field_is_editable-0").not("#rm_field_value_heading");
            ...
            case 'Rating' :
                RM_jQ("#rm_rating_config_container").show();
            case 'Country' :
            case 'Gender' :
            case 'Timezone' :
            case 'Language' :
            ...
            case 'PGAvatar' :            
            case 'Website' :
                var object = RM_jQ(".rm_static_field").not(".rm_required, #rm_field_is_required-0, #rm_field_helptext_container, #rm_field_is_editable-0,#existing_user_meta");
                break;
...
//                RM_jQ("#rm_product_notice").show();
                break;

            case 'Map' :
            case 'Address' : 
                var object = RM_jQ(".rm_static_field").not("#rm_field_type_select_dropdown, #rm_field_is_editable-0, #rm_field_label, #rm_field_is_required-0, #rm_field_helptext_container, #rm_field_show_on-0, #existing_user_meta");
    ...
    RM_jQ.setup_pricing_fields_visibility = function (field_type) {
        ...
        var all_elem = RM_jQ(".rm-static-field");
        all_elem.removeClass("rm_prevent_empty");
        ...
        switch (field_type) {
            ...
            case 'fixed':
                ...
                RM_jQ('#id_allow_quantity-0').prop('disabled', false);
                break;
            ...
            case 'userdef':
                ...
                RM_jQ('#id_paypal_field_value_no').prop('required', false);
                RM_jQ('#id_block_fields_for_dd_multisel').hide();
                RM_jQ('#id_block_fields_for_fixed, #id_allow_quantity_container').hide();
                RM_jQ('#id_allow_quantity-0').prop('disabled', true);
                break;
        }
    ...
    RM_jQ.rm_do_action = function (form_id, slug) {
        ...
        } else if(slug == 'rm_submission_update_payment') {
            var has_error = false;
            var selected_subs = RM_jQ("input[name='rm_selected[]']:checked").map(function(){return parseInt(RM_jQ(this).data('pay-id'));}).get();
            selected_subs.forEach(function(item, index) {
                if(item > 0) {
                    var data = {
                        action: 'rm_olp_update_payment',
                        pproc_id: item,
                        pay_status: 'success',
                        pay_note: 'Payment status updated by site admin'
                    };
        ...
        } else if(slug == 'rm_submission_mark_sub_unread') {
            var has_error = false;
            var selected_subs = RM_jQ("input[name='rm_selected[]']:checked").map(function(){return parseInt(RM_jQ(this).val());}).get();
            if(selected_subs.length !== 0) {
                var data = {
                    action: 'rm_mark_submission_unread',
                    slug: slug,
                    sub_ids: selected_subs
                };
 ...
 RM_jQ.rm_test_email_handlers = function (data) {
     alert(data);
...
function rmPrDashboardUserImages() {
