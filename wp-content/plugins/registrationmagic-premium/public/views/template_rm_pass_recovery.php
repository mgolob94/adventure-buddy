...
if($data->form_type=='rm_recovery_form'){
    ...
    if(isset($data->valid_email) && $data->valid_email==1){
        ...
        $btn_label= !empty($data->options['rec_btn_label'])?$data->options['rec_btn_label']:__('Reset Password', 'registrationmagic-addon');
    ...
    if(isset($data->invalid_copy_token) && $data->invalid_copy_token==1){
        $recovery_link= get_permalink($data->options['recovery_page']);
    ...
    $form->addElement(new Element_Textbox(__('Security Token', 'registrationmagic-addon'), "token_val", array("required" => "1","class"=>'', "placeholder" => '','style'=>isset($data->design['style_textfield'])?$data->design['style_textfield']:null)));
    
    $btn_label= !empty($data->options['rec_tok_sub_label'])?$data->options['rec_tok_sub_label']:__('Proceed', 'registrationmagic-addon');
    $form->addElement(new Element_Button($btn_label, "submit", array("id" => "rm_submit_btn", "class" => "rm_btn", "name" => "submit",'style'=>isset($data->design['style_btnfield'])?$data->design['style_btnfield']:null)));
    ...
    if(empty($data->password_updated)){
        ...
        $btn_label= !empty($data->options['rec_pass_btn_label'])?$data->options['rec_pass_btn_label']:__('Change Password', 'registrationmagic-addon');
        $form->addElement(new Element_Button($btn_label, "submit", array("id" => "rm_submit_btn", "class" => "rm_btn", "name" => "submit",'style'=>isset($data->design['style_btnfield'])?$data->design['style_btnfield']:null)));
    }
