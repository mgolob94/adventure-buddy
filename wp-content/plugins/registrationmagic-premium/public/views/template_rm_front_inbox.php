...
if(isset($data->section_style)){
    ...
    $class= 'rmagic rm-single-tabs rm-single-inbox-tabs';
}
?>

        <div class="rmagic-table <?php echo $class;?>" id="rm_inbox_tab" style="<?php echo $section_style; ?>">
            <div class="rm-user-row rm-icon rm-inbox-icon dbfl">
                <i class="material-icons rm-bg" data-rm_apply_acc_bgcolor="true"><?php  _e('mail', 'registrationmagic-addon') ?></i>
            </div>
            ...
            <?php 
             if (!isset($data->inbox->mails) || (!is_array($data->inbox->mails) || count($data->inbox->mails) < 1)):
            ...
            <table class="rm-user-data rm-inbox-data">
                ...
                //New Inbox
                if (isset($data->inbox->mails) && is_array($data->inbox->mails)):
                    $i = 0 ;
                    ...
                    <tr class="rm_email_detail_container" id="rm_email_detail_view_<?php echo $i; ?>" style="display: none">
                        <td colspan="4">
                            <div class="rm_email_detail dbfl">
                                <div class="rm_email_detail_label difl"><?php  _e('Date', 'registrationmagic-addon') ?></div> 
                                <div class="rm_email_detail_body difl">   
                                    <?php echo $mail->sent_on; ?>
                                </div>
                            </div>
                            <div class="rm_email_detail dbfl">
                                <div class="rm_email_detail_label difl"><?php  _e('Subject', 'registrationmagic-addon') ?></div>
                                <div class="rm_email_detail_body difl">
                                    <?php echo htmlspecialchars_decode($mail->sub);?>
                                </div>
                            </div>
                            <div class="rm_email_detail dbfl">
                                <div class="rm_email_detail_label difl"><?php  _e('Message', 'registrationmagic-addon') ?></div>
                                <div class="rm_email_detail_body difl">
            ...
            if (isset($data->inbox->total_pages_inbox) && $data->inbox->total_pages_inbox > 1):
                ?>
