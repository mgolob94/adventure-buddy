...
// Checking if captcha is enabled
          ...
          $check_setting=null;
            ...
            if ($data->is_mailchimp_enabled == false && $data->form->form_options->form_is_opt_in_checkbox == 5 )
            {
           ...
           if ($data->is_ccontact_enabled == true && $data->form->form_options->form_is_opt_in_checkbox_cc == 1)
            {
                $form->addElement(new Element_Checkbox('', 'rm_subscribe_cc', array(1 => $data->form->form_options->form_opt_in_text_cc ? : RM_UI_Strings::get('MSG_SUBSCRIBE_CC'))));
            }
            $form->addElement(new Element_HTML('</div>'));
            ...
            $form->addElement(new Element_Button($btn_label != "" ? $btn_label : __('Submit', 'registrationmagic-addon'), "submit", array("bgColor" => "#$data->submit_btn_bgcolor", "fgColor" => "#$data->submit_btn_fgcolor")));
            ...
            else
            {
                ...
                if ($data->expiry_details->state !== 'perpetual')
                {
                    ...
                    else
                    {
                        ...
                        {
                             case 'both':
                                $exp_str .= '<div class="rm-formcard-expired">' .sprintf(esc_html__('%d out of %d filled and %d days to go','registrationmagic-addon' ),($data->expiry_details->sub_limit - $data->expiry_details->remaining_subs),$data->expiry_details->sub_limit,$data->expiry_details->remaining_days).' </div>';
                            ...
                            case 'subs':
                                ...
                                if ($data->expiry_details->criteria == 'subs')
                                    $exp_str .= '<div class="rm-formcard-expired">' . sprintf(esc_html__('%d out of %d filled.','registrationmagic-addon'),($data->expiry_details->sub_limit - $data->expiry_details->remaining_subs),$data->expiry_details->sub_limit).'</div>';
                                break;

                            case 'date':
                                $exp_str .= '<div class="rm-formcard-expired">' .sprintf(esc_html__('%d days to go.','registrationmagic-addon'),$data->expiry_details->remaining_days). '</div>';
                                break;
                        }
                    }
