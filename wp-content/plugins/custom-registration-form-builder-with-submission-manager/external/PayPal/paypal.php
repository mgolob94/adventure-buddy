...
class rm_paypal_class {
	...
	function __construct() {
		...
		$this->sandbox = 'no';
                ...
                $this->paypal_modern_method = false;
	}
    ...
    public function toggle_modern_method($trueorfalse){
            $this->paypal_modern_method = $trueorfalse;
    }
    public function toggle_client_id($client_id){
            $this->paypal_client_id = $client_id;
    }    
	...
	public function submit_paypal_post() {
		...
		if (isset($this->paypal_mail))echo "<input type='hidden' name='business' value='".esc_attr($this->paypal_mail)."'/>\n";
		...
		echo "<head><title>".__('Processing Payment...','custom-registration-form-builder-with-submission-manager')."</title></head>\n";
		echo "<body>\n";
                echo "<div id='rm_paypal_order_container'>\n";
                echo "<div id='rm_paypal_order_success' style='display:none;'><img src='".RM_IMG_URL."/paypal-success.png'/></div>\n";
                echo "<div id='rm_paypal_order_area'>\n";
		echo "<h2>".__('Your Order','custom-registration-form-builder-with-submission-manager')."</h2>\n";
		echo "<table class='rm_paypal_order_details' style='width:100%'>\n";
                echo "<thead><tr>\n";
                echo "<th>".__('Product','custom-registration-form-builder-with-submission-manager')."</th>\n";
                echo "<th>".__('Quantity','custom-registration-form-builder-with-submission-manager')."</th>\n";
                echo "<th>".__('Price','custom-registration-form-builder-with-submission-manager')."</th>\n";
                echo "<th>".__('Subtotal','custom-registration-form-builder-with-submission-manager')."</th>\n";
                echo "</tr></thead>\n";
                echo "<tbody>\n";
                foreach($pricing_details->billing as $items){
                    echo "<tr>\n";
                    echo "<td>".$items->label."</td>\n";
                    echo "<td>".$items->qty."</td>\n";
                    echo "<td>".RM_Utilities::get_formatted_price($items->price)."</td>\n";
                    echo "<td>". RM_Utilities::get_formatted_price($items->price * $items->qty)."</td>\n";
                    echo "</tr>\n";
                }
                echo "</tbody>\n";
                echo "<tfoot>\n";
                    ...
                    if($pricing_details->total_price > 0){
                        echo "<tr><td></td><td colspan='2'><b>".__('Total','custom-registration-form-builder-with-submission-manager')."</b></td>";
                        echo "<td><b>".RM_Utilities::get_formatted_price($pricing_details->total_price)."</b></td>\n";
                        echo "</tr>\n";
                    }
                echo "</tfoot>\n";
                echo "</table>\n";
                echo "<h3>".__('Available Payment Method:-','custom-registration-form-builder-with-submission-manager')."</h3>";
                echo "<div id='rm_paypal_btn_container'></div>\n";
                echo "</div>";
                echo "</div>";
                echo "<script src='https://www.paypal.com/sdk/js?client-id=".$this->paypal_client_id."&currency=".$currency."'></script>";
                echo "<script>var orders = ".$order_details.";</script>";
                echo "<script>
                    (function ($) {
                        $(window).load(function(e){
                        initialize_paypal_sdk('".$submission_id."','".$payment_id."','".$user_id."','".$btn_color."');
                         });
                })(jQuery);
                </script>";
                echo "</body></html>\n";
