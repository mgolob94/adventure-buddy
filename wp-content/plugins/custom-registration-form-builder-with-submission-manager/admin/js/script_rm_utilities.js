...
function rm_append_field(tag, element) {
    var eleCopy = jQuery(element).parents('#rm_action_container_id').prev().children(tag).first().clone();
    eleCopy.children("input").val('');
    jQuery(element).parents('#rm_action_container_id').prev().append(eleCopy);
...
function rm_load_page(element, page, get_var){
    ...
    if(selected_value=='rm_login_form' && page=='field_manage'){
        ...
        window.location = window.location.href + '&rm_' + get_var + '=' + selected_value;
    } else
...
function initialize_payment_status_filters(){
    jQuery(".rm-auto-status").each(function () {
          //Submission Filter settings
          if (jQuery(this).hasClass('rm-submission-tag')) {
              var urlParams= getUrlParams();
              ...
              jQuery(this).tokens({
                    source : rm_payment_status_js_data.submission_filter_tags,
                    initValue : decodeURIComponent(tags).replace(/\+/g,  " ").split(","),
                    showSuggestionOnFocus : true,
                    onSelect : function(suggestions){
                        var urlParams= getUrlParams();
...
function load_payment_status_js_data(){
    var data = {
            'action': 'rm_load_payment_status_admin_js_data',
            'rm_sec_nonce': rm_admin_vars.nonce
        };
        
        jQuery.post(ajaxurl, data, function (response) {
           rm_payment_status_js_data= JSON.parse(response);
           initialize_payment_status_filters();
        });
}   
